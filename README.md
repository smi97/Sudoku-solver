...
В качестве контейнера для хранения судоку используется двухмерный `std::deque` целых числел. Он удобен тем, что позволяет легко производить последовательный обход матрицы и имеет сложность добавления О(1). Для работы методов проверки ряда, столбца и квадрата3х3 используются контейнеры `std::set`, а в методе, который проверяет одну ячейку с помощью вышеупомянтых - `std::unordered_set`. При использовании именно таких контейнеров достигается самая быстрая работа программы, что демонстрируют замеры времени работы программы:
![](img/time_tests.png)
